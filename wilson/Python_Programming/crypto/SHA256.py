...
def pad_message(message):
    # change the message into series of bytes
    padded_message = 0
    for c in message:
    ...
    L = len(message) * 8
    # append filter zeros so that L + 1 bit + 64 = a multiple of 512
...
def SHA256(message):
    ...
    for i in range(chunk_count):
        ...
        # extend the word schedule using the σ (sigma)  functions
        for w in range(16, 64):
            word = (word_array[w - 16] + word_array[w - 7] + sig0(word_array[w - 15]) + sig1(
                word_array[w - 2])) % 2 ** 32
            word_array.append(word)
        # start of temporary hash values using initial hash list
        ...
        # set the hash list using final temp values
        for h in range(8):
...
# testing
"""
msg = "Jayden is gonna rock your world"
